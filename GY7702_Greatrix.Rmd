...
```{r, echo=FALSE, message=FALSE}
kable(
  (Dudley_complete_covid_data[1:10,]), 
  "simple")
```
...
```{r}
Dudley_covid_development_pipe_test <- 
  Dudley_day_before %>%
...
```{r, echo=FALSE, message=FALSE}
Knitr::kable(
  Dudley_covid_development_pipe_test[1:10,], 
  "simple"
  )
``` 

By way of an explanation, ```newCases_day_before``` 
states the cases the day before ```specimen_date. ```newCasesBySpecimenDate``` 
...
percentage_change <- ggplot2::ggplot(
  ...
  # Specify the data 
  data = Dudley_covid_development_pipe_test, mapping = aes(
...
```{r, echo = FALSE, message = FALSE}
knitr::kable(cov_pop[1:10,], "simple")
