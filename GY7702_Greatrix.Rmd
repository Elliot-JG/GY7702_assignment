...
Dudley_day_before_pipe_test %>%
  ...
  # Join [area]_day_before with Dudley_complete_covid_data, specimen_date is equal to the day_before 
...
Finally, ```percentage_of_new_cases``` returns ```Inf``` when ```newCases_day_before``` is 0. These can be cleaned with        
```{r}
Dudley_day_before_pipe_test[!is.finite(percentage_of_new_cases)] <- 0
```
